...
def visualize(data, metadata, figsize=None, rowcol=None):
    cmap = setup_greener_color()
    
    Nmeas = data.shape[0]
    if figsize is None:
        fig = plt.figure(figsize=(5,8*Nmeas//6))
    else:
    ...
    for dd in range(Nmeas):
        if rowcol is None:
            plt.subplot(6,np.ceil(Nmeas/6),dd+1)
        else:
            plt.subplot(rowcol[0],rowcol[1],dd+1)
